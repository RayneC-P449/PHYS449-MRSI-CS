classdef SignalModel
   ...
   methods
       ...
       function [kspace, t, ppm] = extract_kspace(obj, metab_bases, seq_params, phantom, zslice, voxrange, snr, beta, B0_map)
           ...
           water_fid = zeros(Nx,Ny,seq_params.n);
           for sc = zslice
               ...
               Aw = Aw / sum(Aw);
               for w = 1:5
                   water = reshape(water_bases{w}, 1, 1, []);
                   water_fid = water_fid + Aw(w) * (phantom.water_data(:,:,sc,1) .* water) .* exp(reshape(-t_vect,1,1,[]) ./ phantom.water_data(:,:,sc,2));
               end
           end
           wspace = zeros(size(kspace));
           for vx = 1:voxrange(1)
               for vy = 1:voxrange(2)
